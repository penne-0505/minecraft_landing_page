...
- **Title**: [Perf] Optimize Google Fonts loading for LP
- **ID**: UI-Perf-21
- **Priority**: P1
- **Size**: S
- **Area**: UI
- **Dependencies**: []
- **Goal**: LPのGoogle Fonts読み込みが非ブロッキング化され、Lighthouseの「レンダリングをブロックしているリクエスト」が改善する。
- **Steps**:
  1. [x] `index.html` のGoogle Fonts読み込みを非ブロッキング化する（preload + onload + noscript）
  2. [ ] LPのフォント指定（`JoinLanding.jsx`）に影響がないことを確認する
  3. [ ] Lighthouseで再計測し、フォント由来のブロッキング時間が減っていることを確認する
- **Description**: LPで利用するGoogle FontsのCSS読み込みを非ブロッキング化してFCP/LCPの待ち時間を削減する。
- **Plan**: None

- **Title**: [Perf] Add smaller image variants for LP assets
- **ID**: UI-Perf-22
- **Priority**: P1
- **Size**: S
- **Area**: UI
- **Dependencies**: []
- **Goal**: LP画像の`srcset`に小さいバリアントが追加され、Lighthouseの「適切なサイズの画像」警告が軽減される。
- **Steps**:
  1. [x] `src/assets/images/landing/` の各画像に小さなバリアント（例: 320/480）を追加する
  2. [x] `src/data/lpImages.js` の`srcset`に追加バリアントを登録する
  3. [x] `_docs/guide/design/lp_image_bundling.md` に新しいバリアント規約を反映する
  4. [ ] Lighthouseで「適切なサイズの画像」の改善を確認する
- **Description**: LPで使用する画像により小さなバリアントを追加し、表示サイズに近い画像を配信できるようにする。
- **Plan**: None
